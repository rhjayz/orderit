...
export const dateRange = (filterButton:FilterType): {startDate: Date, endDate: Date} => {
...
export const filterCard = asyncHandler(async (req: Request, res: Response) => {
        ...
        const dataChecked = parseDate && !isNaN(parseDate.getTime()) ? parseDate : startDate;
...
export const chartDashboard = asyncHandler(async (req: Request, res: Response) => {
    ...
    const GroupingYearData = (transaction: transactionProps[]) => {
        return transaction.reduce<Record<number, periodeDataProps>>((acc, trx) => {
            const year = new Date(trx.createdAt).getFullYear();
            const periode = Math.floor(year / 8) * 8;

            if (!acc[periode]) acc[periode] = { periode, years: {} };
            if (!acc[periode].years[year]) acc[periode].years[year] = { year, totalPrice: 0, transaction: [] };
    ...
    const existingYears = new Map(formattedData.flatMap(d => d.years.map(y => [y.year, y.totalPrice])));

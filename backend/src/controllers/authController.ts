...
export const verifyAccount = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!process.env.SECRET_KEY)
    {
        ...
        const decoded = jwt.verify(reqToken, process.env.SECRET_KEY) as { email: string };
...
export const resetPassword = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!resetData) {
        return res.redirect(`${process.env.FRONTEND_URL}/forgotpassword?error=Token invalid`);
    }

    if (!resetData || resetData.createdAt < new Date()) {
        return res.redirect(`${process.env.FRONTEND_URL}/forgotpassword?error=Token expired`);
    }

        res.redirect(`${process.env.FRONTEND_URL}/newpassword?token=${paramsToken}`); 
...
export const updatePassword = asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!where) {
        res.status(400).json({ message: "You don't have link to change ur password" });
        return;
    }
...
export const register = asyncHandler(async (req: Request, res: Response) => {
        ...
        if (!process.env.SECRET_KEY) { 
            res.status(500).json({message:"Server misconfiguration: SECRET_KEY not set."})
            return;
        } 
        ...
        const state = await prisma.user.create({
            data : { name, email, password: hashedPassword, secretToken, isVerified: false },
        });
        
        await sendVerification(email, secretToken);
        res.status(201).json({message:"Success! Please check your email for verification."})
});
    ...
    export const authGoogle = asyncHandler(async (req: Request, res: Response) => {
        ...
        res.cookie("token", token, {
            httpOnly: false,
            secure: false, // true kalau di production, false kalau di localhost
            sameSite: "lax", // agar tetap aman di production
            maxAge: 24 * 60 * 60 * 1000,
        });
...
export const authCheckGoogle = asyncHandler(async (req: Request, res: Response) => {
...
export const login = asyncHandler(async (req: Request, res: Response) => {
    ...
    res.status(200).json({message:`Login Success, welcome ${users.name}`, token})
});
